ggheat = function(file.name, dat, colors, width.adjust=2.1) {
  library(ggplot2)
  library(cba)
  

dat = dat[hclust(dist(dat))$order,]
#dat = dat[hclust(dist(dat,method="spearman"))$order,]
...
base_size=8
g = ggplot(data=dat.m)+geom_rect(aes(xmin=colInd-1,xmax=colInd,ymin=rowInd-1,ymax=rowInd,fill=value,colour=value))
...
#   p = ggplot(dat.m, aes(x=Var2, y=Var1)) +
#  labs(x="",y="") + 
#  scale_x_discrete(expand=c(0,0)) + 
#  scale_y_discrete(expand=c(0,0)) +
#  geom_tile(aes(fill=value,colour=value)) +
#  opts(axis.ticks=theme_blank(), axis.text.x=theme_blank(), axis.title.x=theme_blank(), axis.text.y=theme_text(size=6, hjust=1)) +
#  scale_fill_gradient2(low=colors[1],mid=colors[2],high=colors[3],midpoint=0) + 
#  scale_colour_gradient2(low=colors[1],mid=colors[2],high=colors[3],midpoint=0)
